...
class sidebar_generator {
	    ...
	    if(is_array($sidebars)){
			foreach($sidebars as $sidebar){
				...
				register_sidebar(array(
					...
					'before_widget' => '<div id="%1$s" class="widget widget-item %2$s">',
					'after_widget' => '</div>',
	...
	public static function admin_menu(){
		//add_theme_page('Sidebar Manager', 'Sidebar Manager', 'manage_options', 'sidebar_manager', array('sidebar_generator','admin_page'));
	}
	...
	public static function get_sidebar($name="0"){
		if(!is_singular()){
			if($name != "0"){
				...
				dynamic_sidebar('hb-default-sidebar');
			}
			...
			if(is_array($selected_sidebar) && !empty($selected_sidebar)){
				for($i=0;$i<sizeof($selected_sidebar);$i++){					
					...
					if($name == "0" && $selected_sidebar[$i] == "0" &&  $selected_sidebar_replacement[$i] == "0"){
						...
						dynamic_sidebar('hb-default-sidebar');//default behavior
						$did_sidebar = true;
						break;
