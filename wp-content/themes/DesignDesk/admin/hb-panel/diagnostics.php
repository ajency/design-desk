...
<div class="wrap about-wrap hb-alt-wrap">
 ...
 <table class="hb_diagnostics_table widefat" cellspacing="0">
	<thead>
		<tr>
			<th colspan="2"><strong><?php _e("WordPress", "hbthemes"); ?></strong></th>
		</tr>
	</thead>
	<tbody>
		<tr>
			<td class="first"><?php _e("Home URL", "hbthemes") ?>: </td>
			<td><?php echo home_url(); ?></td>
		</tr>
		<tr>
			<td class="first"><?php _e( 'Site URL', 'hbthemes' ); ?>:</td>
			<td><?php echo site_url(); ?></td>
		</tr>
		<tr>
			<td class="first"><?php _e("WordPress Version", "hbthemes") ?>: </td>
			<td><?php echo get_bloginfo('version'); ?></td>
		</tr>
		<tr>
			<td class="first"><?php _e("WordPress Language", "hbthemes") ?>: </td>
			<td><?php echo get_bloginfo('language'); ?></td>
		</tr>
		<tr>
			<td class="first"><?php _e("Admin Email", "hbthemes") ?>: </td>
			<td><?php echo get_bloginfo('admin_email'); ?></td>
		</tr>

		<tr>
			<td class="first"><?php _e("Installed Plugins", "hbthemes") ?>: </td>
			<td><?php
				...
				foreach ( $active_plugins as $plugin ) {
					...
					if ( ! empty( $plugin_data['Name'] ) ) {
						...
						if ( ! empty( $plugin_data['PluginURI'] ) ) {
							$plugin_name = '<a href="' . esc_url( $plugin_data['PluginURI'] ) . '" title="' . __( 'Visit plugin homepage' , 'hbthemes' ) . '">' . $plugin_name . '</a>';
						}

						$wc_plugins[] = $plugin_name . ' ' . __( 'by', 'hbthemes' ) . ' ' . $plugin_data['Author'] . ' ' . __( 'version', 'hbthemes' ) . ' ' . $plugin_data['Version'] . $version_string;
