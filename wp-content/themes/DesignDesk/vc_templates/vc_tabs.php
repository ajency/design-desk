...
$tabs_nav = '';
$tabs_nav .= '<ul class="nav nav-tabs clearfix wpb_tabs_nav">';
foreach ( $tab_titles as $tab ) {
    preg_match('/title="([^\"]+)"(\stab_id\=\"([^\"]+)\"){0,1}(\sicon\=\"([^\"]+)\"){0,1}/i', $tab[0], $tab_matches, PREG_OFFSET_CAPTURE );
    if(isset($tab_matches[1][0])) {
//    	echo $tab_matches[5][0];
        $tabs_nav .= '<li><a href="#tab-'. (isset($tab_matches[3][0]) ? $tab_matches[3][0] : sanitize_title( $tab_matches[1][0] ) ) .'" data-toggle="tab">';

        if ( $tab_matches[5][0] )
        	$tabs_nav .= '<i class="' . $tab_matches[5][0] . '"></i>';
...
$output .= "\n\t".'<div class="hb-tabs-wrapper clearfix '.$css_class.'" data-interval="'.$interval.'">';
