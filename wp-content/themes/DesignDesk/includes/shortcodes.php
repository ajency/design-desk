...
function hb_toggle_shortcode($params = array(), $content = null) {
	...
	if ( $title ) {
		$output .= $title . '<i class="fa fa-angle-right"></i>';
	}
...
function hb_faq_shortcode($params = array(), $content = null) {
	...
	if ( $queried_items->have_posts() ) : while ( $queried_items->have_posts() ) : $queried_items->the_post();
		...
		$output .= '<div class="hb-accordion-single">';
		$output .= '<div class="hb-accordion-tab"><i class="hb-moon-plus-circle"></i> ' . get_the_title() . '<i class="fa fa-angle-right"></i></div>';
...
function hb_fw_map_embed_shortcode($params = array(), $content = null) {
	...
	$output .= '<div class="hb-gmap-map" data-show-location="' . $data_show_location . '" data-map-level="' . $zoom . '" data-map-lat="' . $latitude . '" data-map-lng="' . $longitude . '" data-map-img="' . $custom_pin . '" data-overlay-color="';
...
function hb_blog_carousel_shortcode($params = array(), $content = null) {
	...
	$output .= '<a href="#" class="previous"><i class="fa fa-angle-left"></i></a>';
	$output .= '<a href="#" class="next"><i class="fa fa-angle-right"></i></a>';
	$output .= '</div>';
...
function hb_portfolio_carousel_shortcode($params = array()) {
	...
	while ( $queried_items->have_posts() ) : $queried_items->the_post();
		...
		if ( $style == "descriptive" ) {
			...
			$output .= '<a href="' . $perma . '" class="float-right details-link">' . __('Details' , 'hbthemes') . ' <i class="fa fa-angle-right"></i></a>';
			$output .= '</div>';
		}
	...
	foreach ($all_socials as $social_network) {
		if ( $$social_network ) {
			if ($social_network == 'google_plus') {
				...
				$new_soc_net = 'feed-2';
			} elseif ($social_network == 'envelop') {
				$new_soc_net = 'envelope';
			} else {
				$new_soc_net = $social_network;
			}
			
			if ( $new_soc_net != 'behance' && $new_soc_net != 'vk' ){
				$output .= '<li class="' . $new_soc_net . '"><a href="' . $$social_network . '" target="' . $new_tab . '"><i class="fa fa-' . $new_soc_net . '"></i><i class="fa fa-' . $new_soc_net . '"></i></a></li>';
			} else {
				$output .= '<li class="' . $new_soc_net . '"><a href="' . $$social_network . '" target="' . $new_tab . '"><i class="fa fa-' . $new_soc_net . '"></i><i class="fa fa-' . $new_soc_net . '"></i></a></li>';
			}
		}
...
Example: hb-moon-apple-fruit. Example for character: $", "js_composer")
		...
		wpb_map( array(
			...
			"base" => "fullwidth_section",
		  	...
		  	"category" => __('Deprecated', 'js_composer'),
		...
		wpb_map( array(
		    ...
		    "params"	=> array(
			    ...
			    array(
				      ...
				      "param_name" => "text",
				      "value" => __("<p>I am text block. Click edit button to change this text. Lorem ipsum dolor sit amet, consectetur adipiscing elit.</p>", "js_composer")
			    ),
			    array(
				      "type" => "textfield",
