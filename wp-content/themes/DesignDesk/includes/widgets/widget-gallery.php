...
class hb_gallery_widget extends WP_Widget {
	...
	function HB_Gallery_Widget() {
	
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'gallery-widget', 'description' => __('A widget that displays your latest gallery posts.', 'hbthemes') );
		$control_ops = array ();
		/* Create the widget. */
		$this->WP_Widget( 'hb_gallery_widget', __('[HB-Themes] Gallery Grid (Latest)','hbthemes'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		/* Display Dribbble */
			...
			if ( $thumb ) {
				$counter--;
				if ($counter>=0) {
					echo '<li><a href="'.$full_image[0].'" title="'.get_the_title().'" rel="prettyPhoto[gallery_'.$gallery_rel.']">';
