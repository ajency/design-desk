 ...
 * Description: A widget that displays your most liked posts.
 * Version: 1.0
 * Author: HB-Themes
 ...
 */
add_action( 'widgets_init', 'hb_most_liked_posts_widget' );

/*
 * Register widget.
 */
function hb_most_liked_posts_widget() {
	register_widget( 'HB_Most_Liked_Posts_Widget' );
}

/*
 * Widget class.
 */
class hb_most_liked_posts_widget extends WP_Widget {
	...
	function HB_Most_Liked_Posts_Widget() {
		...
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'hb_most_liked_posts_widget', 'description' => __('A widget that displays most liked blog posts.', 'hbthemes') );
		$control_ops = array ();
		/* Create the widget. */
		$this->WP_Widget( 'hb_most_liked_posts_widget', __('[HB-Themes] Post List - Most Liked Widget','hbthemes'), $widget_ops, $control_ops );
	}
		...
		<ul class="most-liked-list">
			...
			?>
				<li>
					...
					<a href="<?php the_permalink(); ?>" class="like-count-num"><?php echo number_format($likes); ?></a>
				</li>
			<?php
			endwhile;
			?>
