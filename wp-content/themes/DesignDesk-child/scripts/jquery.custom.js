...
function hb_placeholder_fixes() {
	...
	$j('input[type=text],input[type=email],input[type=search],input[type=password]').simplePlaceholder();
}

/* SuperFish initialization */
function hb_menu_init(){
	$body = $j('body');
	$main_wrapper = $j('#main-wrapper');
	$j(".sf-menu").superfish({
		delay: 50,
		speed: 200,
		speedOut: 100,
	...
	if ($j('#main-nav').length && !$j('#main-nav').hasClass('sf-subbed')){
		$j('#main-nav li').each(function(){
			...
			if($j(this).find('> ul').length > 0) {
				 ...
				 $j(this).find('> a').append('<i class="sf-sub-indicator fa fa-angle-right"></i>');
			}
		});
	}
...
function hb_menu_search() {
	...
	if ( $main_nav.length && $body.hasClass('has-side-section') && hb_side_panel_added == 0 ){
		$main_nav.append('<li id="hb-panel-opener" class="external"><a class="hb-open-side-section" href="#"><i class="fa fa-bars icon-bars"></i></a></li>');
...
/* jQuery Parallax */
function hbParallax(e) {
    var t = $j(e).visible(true);
    if (t && $wind.width() > 1024) {
        var n = parseInt($j(e).offset().top);
        var r = n - $wind.scrollTop();
        var i = -(r / 1.7);
        var s = "50% " + i + "px";
		hb_gs.TweenMax.to(e, 0, {
			css:{ 'backgroundPosition': s },
	...
	if ( $j('#hb-page-title.parallax').length && !$body.hasClass('mobile') && navigator.userAgent.match(/iPad/i) == null){
		...
		$wind.scroll(function(){
			...
			if(!$body.hasClass('mobile') && navigator.userAgent.match(/iPad/i) == null){
				hb_gs.TweenLite.to($j('.hb-image-bg-wrap'), 0, { y: $scroll_val_img, ease:hb_gs.Linear.easeNone, autoRound:false});
				hb_gs.TweenLite.to($j('.hb-page-title, .breadcrumbs-wrapper'), 0, { y: $scroll_val, opacity: $opac, ease:hb_gs.Linear.easeNone, autoRound:false});
			}
...
function onSuccessSend(results){
	window.location.href = document.location.origin+"/thank-you";
...
function hb_init_standard_gallery(){
	...
	if ( $standard_gallery_masonry.length ){
		...
		$j('ul.filt-tabs > li > a').click(function() {
			...
			var all_images = document.getElementsByClassName('standard-gallery-item-wrap');
			//console.log(all_images);
			var i;
			var selector = $j(this).attr('data-filter');
			document.getElementById('standard-gallery-masonry').style.height = "50px";
			document.getElementById('standard-gallery-masonry').innerHTML = "<center><img src='../wp-content/themes/DesignDesk/images/prettyPhoto/default/loader.gif' alt='Loading!!'></center>";
			jQuery.ajax({
                    type: "POST",
                    url: "../wp-admin/admin-ajax.php",          ///////the function is in Highendwp-child functions.php name of function-> my_action
                    data: {
						'action': 'my_action',
						'name': selector      // We pass php values differently!
					},
                    success: function(data) {
                        document.getElementById('standard-gallery-masonry').innerHTML = data;
                        var newselector = selector.substring(1, selector.length);
   						var left = 0;
   						var top = 0;
   						var w = window.innerWidth;
						var h = window.innerHeight;
						var filtered_images = document.getElementsByClassName(newselector);
						for (i = 0; i < filtered_images.length; i++) {
							if(left > w)   //768
							{
								top = top + h/2;
								left = 0;
						...
						top += h/4;
						document.getElementById('standard-gallery-masonry').style.height = top.toString()+"px";
						make_changes_after_ajax_call_to_filter_tab();
						runprettyphoto();
    				},
   	        	});
...
function make_changes_after_ajax_call_to_filter_tab(){
	/* Hover Functions */
	$j('.hb-gal-standard-img-wrapper').hover(function(){
		var $that = $j(this);
		$that.find('.item-overlay-text-wrap').stop().animate({
...
function runprettyphoto() {
	$j(".theprettygallery").each(function() {
		var thepostID = $j(this).data('theid');
		var hashtext = $j(this).data('title');
		hashtext = hashtext.replace(/([~!@#$%^&*()_+=`{}\[\]\|\\:;'<>,.\/? ])+/g, '-').replace(/^(-)+|(-)+$/g,'').toLowerCase();
		$j(this).addClass('thetitle_' + hashtext + '_' + thepostID);
	});

	if (window.location.hash) {
		var current_hashtext = window.location.hash.substr(1);
		setTimeout(function() {
			$j('.thetitle_' + current_hashtext).trigger('click');
		}, 50);
	}
	...
	$j("a[rel^='prettyPhoto']").click(function() {
		var thepostID = $j(this).data('theid');
		var hashtext = $j(this).data('title');
		hashtext = hashtext.replace(/([~!@#$%^&*()_+=`{}\[\]\|\\:;'<>,.\/? ])+/g, '-').replace(/^(-)+|(-)+$/g,'').toLowerCase();
		...
		var location = $j(this).data('location');
		var category = $j(this).data('category');
		var task = $j(this).next('.theneededdata').find('.nee-thetask').html();
		var desc = $j(this).next('.theneededdata').find('.nee-fulldesc').html();
		var howdyHight;

		$j('body').addClass('pp-is-open');


		$j('.maintitle').html(title);
		$j('.thetop').html(
			'<div class="thetop-s">Location- ' + location + '</div>' +
			'<div class="thetop-s">Area- ' + category + '</div>'
			);
		$j('.ourtaskcontent').html(task);
		$j('.howwedidcontent').html(desc);

		if ($j(window).width() > 991) {
			setTimeout(function() {
				howdyHight =  $j('.pp_content_container .pp_left').height() - ($j('.howwedidcontent').offset().top - $j('.pp_content_container .pp_left').offset().top) - 35;
				$j('.howwedidcontent').height(howdyHight);
			}, 50);
			$j('.page-template-page-gallery-standard-pretty .pp_content_container .pp_left, .page-template-page-gallery-standard-pretty .pp_content_container .pp_right').height($j(window).height() - 60)
		}
	});

	$j(window).resize(function() {
		if ($j(window).width() > 991) {
			var howdyHight =  $j('.pp_content_container .pp_left').height() - ($j('.howwedidcontent').offset().top - $j('.pp_content_container .pp_left').offset().top) - 35;
			$j('.howwedidcontent').height(howdyHight);
			$j('.page-template-page-gallery-standard-pretty .pp_content_container .pp_left, .page-template-page-gallery-standard-pretty .pp_content_container .pp_right').height(jQuery(window).height() - 60)
		} else {
			$j('.howwedidcontent').height('auto');
			$j('.page-template-page-gallery-standard-pretty .pp_content_container .pp_left, .page-template-page-gallery-standard-pretty .pp_content_container .pp_right').height('auto')
		}
	});
}